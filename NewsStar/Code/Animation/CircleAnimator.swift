...
class CircleAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    init(view: UIView, color: UIColor, duration: TimeInterval) {
        let center: CGPoint = CGPoint(x: view.center.x, y: view.center.y + CGFloat(44))
        self.circleOrigin = center
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if mode == .present {
            ...
            circle.layer.cornerRadius = circle.frame.width/2//roundCorners(cornerRadius: circle.frame.width / 2.0)
